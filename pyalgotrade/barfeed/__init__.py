...
class BaseBarFeed(feed.BaseFeed):
    """为 :class:`pyalgotrade.bar.Bar` 提供数据源的基类。

    :param frequency: K线的频率. 有效定义详见 :class:`pyalgotrade.bar.Frequency`.
    :param maxLen: :class:`pyalgotrade.dataseries.bards.BarDataSeries` 类型的值驻留的最大数量。当队列已满，一旦添加新的条目进来，将会在相反位置删除对应数量的条目。
    :type maxLen: int.
    ...
    def getNextBars(self):
        """重写并返回下一个数据源的 :class:`pyalgotrade.bar.Bars` ，如果数据源不再有bars则返回None。

        .. note::
            这是BaseBarFeed的子类，它不应该直接被调用。
        """
        raise NotImplementedError()
    ...
    def getCurrentBars(self):
        """返回当前的 :class:`pyalgotrade.bar.Bars`."""
        return self.__currentBars

    def getLastBar(self, instrument):
        """返回给定品种的 :class:`pyalgotrade.bar.Bar` ，如果不存在则返回None."""
    ...
    def getDataSeries(self, instrument=None):
        """返回指定品种的 :class:`pyalgotrade.dataseries.bards.BarDataSeries` 

        :param instrument: 品种标识。如果为空则返回默认品种的值。
