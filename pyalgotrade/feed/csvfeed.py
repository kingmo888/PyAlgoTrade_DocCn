...
class Feed(BaseFeed):
    """一个从格式化后的CSV文件加载数据的数据源。

    :param dateTimeColumn: 带有datetime列的列名.
    :type dateTimeColumn: string.
    :param dateTimeFormat: datetime的转换格式. datetime.datetime.strptime 将用它对该列进行格式转换.
    :type dateTimeFormat: string.
    :param converter: 一个有两个参数（列名和值）的转换函数。它将转换字符串到其他格式。默认转换为float，如果转换失败则返回string原值。
