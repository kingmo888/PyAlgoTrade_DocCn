...
class BasicBar(Bar):
    # 通过限定BasicBar类的属性只包含以下内容（一种优化方式）来达到最小内存占用。
    __slots__ = (
        '__dateTime',
        '__open',
        '__close',
...
class Bars(object):
    ...
    def __getitem__(self, instrument):
        """返回给定品种的 :class:`pyalgotrade.bar.Bar` ，如果品种未找到则引发异常。"""
        return self.__barDict[instrument]

    def __contains__(self, instrument):
        """布尔值。如果给定品种的 :class:`pyalgotrade.bar.Bar` 可用则返回真，否则假。"""
    ...
    def getBar(self, instrument):
        """返回给定品种的 :class:`pyalgotrade.bar.Bar` ，如果品种未找到则返回None"""
