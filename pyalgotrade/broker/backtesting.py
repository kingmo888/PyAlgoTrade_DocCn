...
class FixedPerTrade(Commission):
    """对整个交易收取固定费用的 :class:`Commission` 类 

    :param amount: 一张订单的费用
    :type amount: float.
    """
...
class TradePercentage(Commission):
    """对整个交易按成交金额比例收费的 :class:`Commission` 类

    :param percentage: 收费比例. 0.01 意味着 1%, 这个比例必须小于1.
...
class Broker(broker.Broker):
    ...
    :param cash: 初始的现金
    :type cash: int/float.
    :param barFeed: 提供bar的bar数据源。
    :type barFeed: :class:`pyalgotrade.barfeed.BarFeed`
    :param commission: 一个负责计算订单佣金的对象
    ...
    def getFillStrategy(self):
        """返回当前的 :class:`pyalgotrade.broker.fillstrategy.FillStrategy` 设置."""
    ...
    def getEquity(self):
        """返回绩效组合。（现金+股票）Returns the portfolio value (cash + shares)."""
